module plix.dev.mailbox.bcm;

import core.volatile : vld, vst;
import builtins : memset, memcpy;

import plix.arch.cache : device_fence, inv_dcache;
...
struct BcmMailbox {
    ...
    void write(ubyte channel, uint data) {
        device_fence();
    ...
    bool process(uint* tag, uint tag_size) {
        // set requested tag
    ...
    bool exec_cmd2(uint tag_id, uint id, ref uint value) {
        struct Data {
            uint id;
            uint value;
        }
    ...
    void set_clock_rate(ClockType clock, uint rate, bool skipturbo) {
        struct Data {
            uint id;
            uint rate;
            uint skipturbo;
    ...
    uint get_max_clock_rate(ClockType clock) {
        struct Data {
            uint id;
            uint rate;
        }
